...
def create_summary_model_table1(out_path, template_table, id_field, field_list):
...
def create_summary_model_table2(out_path, template_table, id_field, field_list):
    arcpy.CreateTable_management(out_path, template_table, "", "")
    out_template = out_path + "/" + template_table
    arcpy.AddField_management(out_template, id_field, "LONG")
    for field in field_list:
...
def field_calculate2(in_feature, id_field, field_list, time_point, expression, result_obj={}):
    feature_field_list = sys_arcpy.get_all_fields(in_feature)
    real_field_list = []
    for field in field_list:
        if field in feature_field_list:
    ...
    for row in cursor:
        ...
        if region_id not in result_obj:
            tmp_obj = {time_point: {}}
            for field in field_list:
                tmp_obj[time_point][field] = {"Sum": 0, "Count": 0}
            result_obj[region_id] = tmp_obj
        region_obj = result_obj[region_id]
        for field in field_list:
            field_value = row.getValue(field)
            region_obj[time_point][field]["Sum"] += field_value
            region_obj[time_point][field]["Count"] += 1
        result_obj[region_id] = region_obj
    del cursor, row
    return result_obj


def calculate_average2(result_obj, num):
    for region_id in result_obj:
        for time_point in result_obj[region_id]:
            for field in result_obj[region_id][time_point]:
                result_obj[region_id][time_point][field]["RealAverage"] = (
                    result_obj[region_id][time_point][field]["Sum"] / num
                )
    return result_obj


def save_table2(out_table, result_obj, id_field, time_point):
    cursor = arcpy.InsertCursor(out_table)
    for region_id in result_obj:
        row = cursor.newRow()
        row.setValue(id_field, region_id)
        tmp_obj = result_obj[region_id][time_point]
        for field in tmp_obj:
            val_aver = tmp_obj[field]["RealAverage"]
            row.setValue(("Average_" + field), val_aver)
        cursor.insertRow(row)
